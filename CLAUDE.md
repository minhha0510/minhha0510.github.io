...
**Current demos:**
- `public/liteodyssey/` — rare-disease diagnostic system landing page. The edit-source-of-truth is a single-file HTML mockup (~1500 lines) kept outside this repo as a local working file. Copy that file over `public/liteodyssey/index.html` when updating. Sibling dirs `assets/vendor/asciinema-player/` and `recordings/public/*.cast` are local-relative asset roots the page expects.
- `public/PhD-scheduler/` — built React app (PhD defense scheduling poll). Source lives outside this repo; only the built assets belong here. Ops doc (local only): `../docs/PHD_SCHEDULER.md` in the parent folder.
